import { getDbUser, getMemoryFacts, getUserTransactions, createGoal, updateGoal, upsertUser, getUserGoals } from "@/db/queries";
...
export async function buildSystemPrompt(userId: string | null, fallbackName: string) {
...
export async function executeToolCall(userId: string, name: string, args: Record<string, unknown>): Promise<string> {
  try {
    switch (name) {
      case "create_goal": {
        ...
        const goal = await createGoal({ userId, name: goalName, targetAmount, emoji });
      ...
      case "update_goal": {
        ...
        const updated = await updateGoal(goalId, userId, data);
    ...
    console.error(`[tools] Tool "${name}" failed:`, toolErr instanceof Error ? toolErr.message : toolErr);
